...
sample_metadata <- fread("data/sample_metadata.csv") %>%
  dplyr::mutate(sex = dplyr::recode(sex, "Male" = 0, "Female" = 1))
...
# sex covariate
BrainSpan_sex <- sample_metadata$sex[match(BrainSpan_exonExpr$sampleID, sample_metadata$sampleID)]
...
pcorr_age <- partialCorr(as.data.table(cbind(BrainSpan_exonExpr[,-"sampleID"], age = BrainSpan_age)), 
                         exons = eventOrder, 
                         covariates = "age")
pcorr_sex <- partialCorr(as.data.table(cbind(BrainSpan_exonExpr[,-"sampleID"], sex = BrainSpan_sex)), 
                         exons = eventOrder, 
                         covariates = "sex")
...
df <- rbind(data.frame(corr = as.vector(abs(corr$r[upper.tri(corr$r, diag=F)])),
            ...
            data.frame(corr = as.vector(abs(pcorr_sex$r[upper.tri(pcorr_sex$r, diag=F)])),
                       covariates = "Controlled for sex", dataset = "Developing brain")
)
...
# sex covariate
Otero_sex <- sample_metadata$sex[match(Otero_exonExpr$sampleID, sample_metadata$sampleID)]
...
pcorr_sex <- partialCorr(as.data.table(cbind(Otero_exonExpr[,-"sampleID"], sex = Otero_sex)), 
                         exons = eventOrder, 
                         covariates = "sex")
...
df <- rbind(df,
  ...
  data.frame(corr = as.vector(abs(pcorr_sex$r[upper.tri(pcorr_sex$r, diag=F)])),
    covariates = "Controlled for sex", dataset = "DM1/Unaffected")
)
...
# sex covariate
GTEx_sex <- sample_metadata$sex[match(GTEx_exonExpr$sampleID, sample_metadata$sampleID)]
...
pcorr_age <- partialCorr(as.data.table(cbind(GTEx_exonExpr[,-"sampleID"], age = GTEx_age)), 
                         exons = eventOrder, 
                         covariates = "age")
pcorr_sex <- partialCorr(as.data.table(cbind(GTEx_exonExpr[,-"sampleID"], sex = GTEx_sex)), 
                         exons = eventOrder, 
                         covariates = "sex")
...
df <- rbind(df,
            ...
            data.frame(corr = as.vector(abs(pcorr_sex$r[upper.tri(pcorr_sex$r, diag=F)])),
                       covariates = "Controlled for sex", dataset = "Adult brain")
)
